 ...
 *
 * Emits Google's RECOMMENDED Recipe fields by construction (the set Search Console
 * flagged as "Improve item appearance"): prep/cook/total time, datePublished, and a
 * per-step name/url/image on every HowToStep (the recipe hero image fills each step,
 * clearing the "image or video in recipeInstructions" notice). The policy-sensitive
 * `rating` (real reviews only) and `video` (real asset only) emit only when present.
 */
