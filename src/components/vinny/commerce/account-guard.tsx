 ...
 * session restore settles, a logged-out visitor is redirected to the (redesigned)
 * /login page (carrying ?next= so they land back where they were). Login is
 * OPTIONAL for the store; guest checkout never routes through here.
 */
...
export function AccountGuard({ children }: { children: ReactNode }) {
  ...
  if (!configured) {
    return <p className="text-neutral-500">Customer accounts are not available yet.</p>;
  }
  if (!ready || !customer) {
